...
`@nanoagent/kernel` executes agent run. `@nanoagent/compose` builds final `RunAgentOptions` from reusable pieces.

```sh
npm install @nanoagent/kernel @nanoagent/compose
```
...
import { withPlugins, withTools, withHooks } from '@nanoagent/compose'
