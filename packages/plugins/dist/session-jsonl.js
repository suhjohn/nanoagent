...
export function withJsonlSession(params) {
    return options => withSaveState(({ state }) => params.repo.saveRun?.(state))(withTurnCompleted(args => params.repo.append(params.sessionId, [
        {
            id: args.turn.turnId,
            type: 'custom',
            message: message('assistant', args.turn.modelResult?.text ?? '')
        }
    ]))(withTurnPrepared(async ({ value }) => {
...
export function projectEntries(entries) {
    return entries.flatMap(entry => {
        ...
        if (entry.type === 'compaction')
            return [
                message('system', `<compaction>\n${entry.summary}\n</compaction>`)
            ];
        return [
            message('system', `<branch_summary>\n${entry.summary}\n</branch_summary>`)
        ];
    });
}
