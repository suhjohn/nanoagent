...
export function withSubagentTools(host) {
    const tools = {
        spawn_agent: {
            description: 'Spawn a subagent for bounded work. Use message or items, and omit model overrides unless explicitly needed.',
            ...
            execute: (input, options) => {
                ...
                if ((message === undefined) === (items === undefined)) {
                    throw new Error('spawn_agent requires exactly one of message or items.');
                }
                if (message !== undefined && message.trim() === '') {
                    throw new Error('message must not be empty.');
                }
                if (record.fork_context === true &&
                    (record.agent_type !== undefined ||
                        record.model !== undefined ||
                        record.reasoning_effort !== undefined)) {
                    throw new Error('Full-history forked agents inherit agent type, model, and reasoning effort.');
                }
                return host.spawn({
                    message,
                    items,
        ...
        send_input: {
            ...
            execute: (input, options) => {
                ...
                const items = inputItemsField(record, 'items');
                if ((message === undefined) === (items === undefined)) {
                    throw new Error('send_input requires exactly one of message or items.');
                }
                if (message !== undefined && message.trim() === '') {
                    throw new Error('message must not be empty.');
                }
        ...
        wait_agent: {
            description: 'Wait for agents to reach a final status. Completed statuses may include final messages.',
            inputSchema: objectSchema({
            ...
            execute: (input, options) => {
                ...
                return host.wait({
                    ...
                    timeoutMs: numberField(record, 'timeout_ms', DEFAULT_WAIT_TIMEOUT_MS, MIN_WAIT_TIMEOUT_MS, MAX_WAIT_TIMEOUT_MS),
                    context: options.experimental_context
                });
            }
        },
        ...
        close_agent: {
            description: 'Close an agent and open descendants when they are no longer needed.',
...
function parseInputItem(raw, name) {
    ...
    if (type === 'text')
        return { type, text: stringField(item, 'text') };
    if (type === 'image')
        return { type, image_url: stringField(item, 'image_url') };
    if (type === 'local_image')
        return { type, path: stringField(item, 'path') };
