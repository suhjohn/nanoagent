const composePlugins =
  (...plugins) =>
  options =>
    plugins.reduce((nextOptions, plugin) => plugin(nextOptions), options)
...
export function withExternalHooks(run) {
  return options =>
    appendCallToolMiddleware(async ({ input, next }) => {
      await run({
        event: {
          type: 'PreToolUse',
      ...
      withTurnPrepared(async ({ args, value }) => {
        await run({
          event: { type: 'UserPromptSubmit', prompt: value.messages },
