...
export function withTaskTool(params) {
    ...
    return withTool(toolName, defineTool({
        description: 'Delegate bounded work to named subagent. Returns task_id so later calls can resume same subagent session.',
        ...
        execute: async (input, options) => {
            ...
            const agent = (await params.agents()).find(agent => agent.name === subagentType);
            if (!agent)
            ...
            const result = await params.run({
                ...
                agent,
                taskId: stringField(record, 'task_id', false),
