...
async function main() {
  ...
  const notesFile = path.join(
    stateDir,
    `${safeName(args.sessionId)}.notes.json`
  )
    ...
    withQuestionTool({
      ask: params => {
        const answers = Object.fromEntries(
          params.input.questions.map(question => [
            question.id,
...
async function readNotes(file: string): Promise<SessionNotes> {
  try {
    const parsed = JSON.parse(await readFile(file, 'utf8')) as SessionNotes
    return {
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (!message) {
    throw new Error(
      'Usage: bun run plugin-chat -- --session <id> [--model openai/gpt-5.4-mini] [--root <dir>] [--state-dir <dir>] <message>'
    )
  }
