...
type ScenarioRuntime = {
  name: ScenarioName
  prompt: string
  system: (toolRounds: number) => string
...
async function runScenario(args: CliOptions & { scenario: ScenarioName }) {
  ...
  const emit = (record: Omit<TraceRecord, 'createdAt' | 'scenario'>) => {
    const entry = {
    ...
    if (args.traceJson) process.stdout.write(`${JSON.stringify(entry)}\n`)
  }
  ...
  let toolRounds = 0
  const base: RunAgentOptions<Context> = {
    state: {
      context: {
  ...
  const options = compose<Context>(
    ...commonPlugins({ args, artifacts, emit }),
    ...scenario.plugins,
    withJsonlSession({ sessionId: args.sessionId, repo }),
    withTraceHooks({ artifacts, emit })
  )(base)

  const textTurns = new Set<number>()
  for await (const event of runAgent(options)) {
    printEvent(event, {
      ...
      onText: text => {
        ...
        if (!args.traceJson) process.stdout.write(text)
      },
      emit
    })
  }
  if (!args.traceJson && artifacts.assistantText) process.stdout.write('\n')
  ...
  await writeFile(
    logFile,
    `${artifacts.trace.map(record => JSON.stringify(record)).join('\n')}\n`
  )
}

async function buildScenario(params: {
  args: CliOptions & { scenario: ScenarioName }
  root: string
  stateDir: string
  templateDir: string
  workspace: string
  artifacts: ScenarioArtifacts
  emit: (record: Omit<TraceRecord, 'createdAt' | 'scenario'>) => void
}): Promise<ScenarioRuntime> {
  if (params.args.scenario === 'core') return coreScenario(params)
  if (params.args.scenario === 'coding') return codingScenario(params)
  if (params.args.scenario === 'orchestration') {
    return orchestrationScenario(params)
  }
  if (params.args.scenario === 'knowledge') return knowledgeScenario(params)
  if (params.args.scenario === 'lsp') return lspScenario(params)
...
async function coreScenario(params: {
  ...
  const goalStore = createMemoryGoalStore<Context>()
  return {
    name: 'core',
    maxToolRounds: 3,
    prompt:
      'Run core plugin scenario. Follow system instructions for each tool round.',
    system: round =>
      [
      ...
      withQuestionTool({
        ask: ask => {
          const answers = Object.fromEntries(
            ask.input.questions.map(question => [
              question.id,
...
async function codingScenario(params: {
  ...
  return {
    ...
    maxToolRounds: 9,
    prompt:
      'Run coding plugin scenario. Follow system instructions for each tool round.',
    system: round =>
      [
...
async function orchestrationScenario(params: {
  ...
  const mcpPlugin = await withMcpTools<Context>({
    listTools: () => [
      {
        server: 'demo',
        name: 'lookup',
  ...
  return {
    ...
    maxToolRounds: 11,
    prompt:
      'Run orchestration plugin scenario. Follow system instructions for each tool round.',
    system: round =>
      [
...
async function knowledgeScenario(params: {
  ...
  return {
    ...
    maxToolRounds: 2,
    prompt:
      'Run knowledge plugin scenario using $open-alpha. Follow system instructions.',
    system: round =>
      [
...
async function compactionScenario(params: {
  ...
  const store = {
    ...
    save: async (_context: Context, entry: unknown) => {
  ...
  return {
    ...
    maxToolRounds: 0,
    prompt:
      'Run compaction plugin scenario. This message intentionally contains enough repeated content to trigger compaction. alpha beta gamma delta epsilon zeta eta theta iota kappa lambda mu nu xi omicron pi rho sigma tau.',
...
function withTraceHooks(params: {
  ...
  return options => ({
    ...
    hooks: {
      ...
      onStreamUpdate: args => {
        const part = args.part as { type?: string; toolName?: string }
      ...
      onToolCallCompleted: args => {
        params.emit({
          ...
          turn: args.turn.turn,
          toolName: args.toolName,
          output: 'output' in args ? (args.output as JsonLike) : undefined,
...
function printEvent(
  ...
  handlers: {
    traceJson: boolean
    hasTurnText: (turn: number) => boolean
    markTurnText: (turn: number) => void
...
function emptyArtifacts(): ScenarioArtifacts {
  return {
    plans: [],
    todos: [],
    answers: [],
...
async function ensureDefaultTemplate(templateDir: string) {
...
function parseScenario(value: string): CliOptions['scenario'] {
  ...
  throw new Error(
    `Unknown scenario "${value}". Expected all or ${SCENARIOS.join(', ')}.`
  )
}
...
function takeBooleanFlag(values: string[], name: string) {
  const index = values.indexOf(name)
