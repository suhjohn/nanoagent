...
type PendingContent = string | undefined
...
type LineMatcher = (left: string, right: string) => boolean
...
export function withApplyPatchTool<CONTEXT extends JsonLike>(params: {
  root: string
  toolName?: string
}): AgentPlugin<CONTEXT> {
  ...
  const tool = {
    description:
      'Apply patch in Begin Patch/End Patch grammar with Add File, Delete File, Update File, Move to, and End of File hunks.',
...
export async function applyPatch(params: {
  ...
  const readPending = (requestedPath: string) =>
    readPendingContent({ root, pending, requestedPath })
...
async function stageHunk(params: {
  ...
  root: string
  pending: Map<string, PendingContent>
  readPending: (requestedPath: string) => Promise<string>
  changedFiles: string[]
}) {
  ...
  if (hunk.movePath) {
    ...
    if (oldFile !== newFile) pending.set(oldFile, undefined)
...
async function flushPending(pending: Map<string, PendingContent>) {
...
function normalizePatchLines(patch: string) {
...
function assertPatchEnvelope(lines: string[]) {
...
function parseAddHunk(
  ...
  startIndex: number,
  limit: number
): { hunk: PatchHunk; index: number } {
  const directive = lines[startIndex] ?? ''
  const filePath = requiredPath(
    directive.slice(ADD_FILE.length),
    directive.trim()
  )
...
function parseUpdateHunk(
  ...
  const directive = lines[startIndex] ?? ''
  const filePath = requiredPath(
    directive.slice(UPDATE_FILE.length),
    directive.trim()
  )
  let index = startIndex + 1
...
}): { chunk: UpdateChunk; index: number } {
  const first = params.lines[params.index] ?? ''
  const chunk: UpdateChunk = { oldLines: [], newLines: [], eof: false }
...
function consumeChunkLine(params: {
  ...
  if (params.parsedLines === 0) {
    throw new Error(
      `Unexpected line in update hunk: ${line}. Lines must start with space, +, or -.`
    )
  }
  return false
}
...
function splitBomAndLines(content: string) {
...
function advancePastContext(params: {
  ...
  if (params.chunk.context === undefined) return params.lineIndex
  ...
  if (contextIndex === -1) {
    throw new Error(
      `Failed to find context '${params.chunk.context}' in ${params.filePath}`
    )
  }
...
function locateOldLines(params: {
  ...
  // EOF in the source.
  if (params.oldLines.at(-1) !== '') return undefined
  const trimmedOld = params.oldLines.slice(0, -1)
  const trimmedNew =
    params.newLines.at(-1) === ''
...
function seekSequence(params: {
  ...
  if (params.pattern.length === 0) return params.start
  if (params.pattern.length > params.lines.length) return -1
...
const LINE_MATCHERS: readonly LineMatcher[] = [
...
function findSequence(params: {
  ...
  if (params.eof && lastStart >= params.start) {
    if (matchAt(params, lastStart)) return lastStart
  }

  for (let index = params.start; index <= lastStart; index++) {
    if (matchAt(params, index)) return index
  }
  return -1
}

function matchAt(
  params: { lines: string[]; pattern: string[]; match: LineMatcher },
  start: number
) {
...
function stripHeredoc(patch: string) {
  const match = patch
    .trim()
    .match(
...
function requiredPath(value: string, line: string) {
  const filePath = value.trim()
  if (filePath) return filePath
  throw new Error(`Missing path in patch directive: ${line}`)
}

function inside(root: string, requestedPath: string) {
