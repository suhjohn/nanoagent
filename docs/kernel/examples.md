...
- [Compact](../../examples/compact/README.md): caller-owned conversation memory, per-turn token usage compaction in `onTurnCompleted`, durable `AgentRunState` persistence.
...
- [Agent server](../../examples/agent-server/README.md): durable sessions, steering, run state, event stream, caller-owned message memory, and streamed Express routes backed by Drizzle and Postgres.
...
Run an example CLI from package directory:

```sh
bun run start "Reply with a one sentence example."
```

Example CLIs use real providers through `packages/kernel/.env`. Set `MODEL` to override default model selection. The agent server example also requires `DATABASE_URL`; it can run as a CLI with `bun run start` or as an HTTP server with `bun run server`.
